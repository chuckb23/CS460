...
int do_tx(struct stty *tty)
{
  int c;
  printf("tx interrupt ");
  if (tty->outspace.value == BUFLEN){ // nothing to do 
     disable_tx(tty);                 // turn off tx interrupt
     return;
  }
  c = (int)tty->outbuf[tty->outhead++];
  tty->outhead %= BUFLEN;
  bputc(tty->port, c);
  V(&tty->outspace);
  return;
...
int usgets(int port,char *y){
  char getline[64];
  int i = 0;
  printf("usgets from port %d\n",port);
  sgetline(port,getline);
  do{
    put_byte(getline[i],running->uss,y++);
    i++;
  }while(getline[i]!='\0');
  return;
}
